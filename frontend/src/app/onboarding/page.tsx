...
import { getTenantByClerkId, updateCompanyName, Tenant } from "@/lib/api";
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    async function loadTenant() {
      ...
      try {
        ...
        // If already has company name and in connecting state, redirect to connection step
        if (tenantData?.company_name && tenantData.onboarding_state !== 'pending') {
          // TODO: redirect to Fivetran connection step
        }
      } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // TODO: Move to next onboarding step (Fivetran connection)
      alert("Company name saved! Next: Connect your data sources");
    } catch (err) {
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-2xl w-full">
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <p className="text-sm text-gray-500">
            Next step: Connect your ERP (Fortnox/Visma) and banking data
          </p>
        </div>
      </div>
    </div>
